...
function buildOverlayHTML(data, detectedLink) {
    ...
    const repeat = REPEAT_OPTIONS.includes(data.repeat) || /^day \d+$/.test(data.repeat) || data.repeat === 'same_weekday' ? data.repeat : 'never'
    ...
    const repeatOptions = allRepeatOptions.map(v => {
        const label = v === 'same_weekday' ? 'Same date every month' : (REPEAT_LABELS[v] || v)
        return `<option value="${v}"${v === repeat ? ' selected' : ''}>${label}</option>`
    }).join('')
